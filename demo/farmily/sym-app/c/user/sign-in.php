...
if(isset($_REQUEST['action']) && ($_REQUEST['action'] == "recover")){
    ...
    $headers .= "От:Семейной Фермы <no-reply@".$_SERVER['HTTP_HOST']."> \n";
	$subject = "Регистрационная информация";
	$message = "<h2>Привет, ".$email ."</h2><br />";
	$message .= "Вы запросили пароль на ".$_SERVER['HTTP_HOST']." с этой электронной почты id. <br />";
	$message .= "Ваш пароль <b>".$pass."</b> .<br />";
	$message .= "&copy; ".$_SERVER['HTTP_HOST'];
	mail($email,$subject,$message,$headers);

			$success = 'Отправленно, проверьте ваш почтовый ящик';
			}else{

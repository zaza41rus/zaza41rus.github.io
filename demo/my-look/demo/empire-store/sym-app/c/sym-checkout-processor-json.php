<?php
	...
	switch($_REQUEST['action']){
		...
		if($auth->is_loggedin()){
			...
			$jsonReturn['error'] = 'Укажите адрес доставки' ;
			}else{
			...
			if(!isset($_REQUEST['gateway'])){
				$jsonReturn['error'] ='Забыли выбрать метод оплаты';
			}
					...
					if(isset($_SESSION['basket']['items'])){
						...
						foreach($_SESSION['basket']['items'] as $item){
							...
							if($item['count'] >= 1){
								...
								$jsonReturn['error'] = $item['name']." по указаному адресу мы не сможем доставить товар";
								}
							}
						}
		...
		$jsonReturn['error'] ='Ошибка: Пользователь не зарегестрирован';
		}
		break;

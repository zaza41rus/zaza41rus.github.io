...
if(isset($_REQUEST['action'])){
			...
			if(isset($_REQUEST['id']) && $_REQUEST['action'] == "edit"){
				...
				<h2>Изменить данные</h2>
				 ...
				 <li><a href="addresses.php" >Мои данные</a></li>
				 <li><a href="addresses.php?action=add" >Добавить новые данные</a></li>
				</ul>
				<hr>
				<?php 
				if($address->msg){
				...
				<form class="form-horizontal" action="addresses.php?id=<?php echo $id; ?>&action=edit" method="post" id="addr-edit">
					...
					<label class="col-md-3 control-label text-right" for="addr-name">Имя:</label><div class="col-md-4">
					<input class="form-control" type="text" name="addr_name" value="<?php echo $addr['name'] ;?>" id="addr-name" ></div></div>
					<div class="form-group">
					<label class="col-md-3 control-label text-right" for="addr-addr">Адрес:</label><div class="col-md-4">
					<textarea  class="form-control" style="width:300px;height:100px;" name="addr_addr" id="addr-addr"><?php echo trim($addr['address']) ;?>
					</textarea></div></div>
					<div class="form-group">
					<label class="col-md-3 control-label text-right" for="addr-c">Страна:</label><div class="col-md-4">
					<input class="form-control" type="text" name="addr_c" value="<?php echo $addr['country'] ;?>" id="addr-c" ></div></div>	<div class="form-group">
					<label class="col-md-3 control-label text-right" for="product-shipping-region">Регион</label><div class="col-md-4">
...
?></select></div></div>
					...
					<div class="form-group">
					<label class="col-md-3 control-label text-right" for="addr-zip">Индекс:</label><div class="col-md-4">
					<input class="form-control" type="text" name="addr_zip" value="<?php echo $addr['zip'] ;?>" id="addr-zip" ></div></div>
					
					<div class="form-group">
					<label class="col-md-3 control-label text-right" for="addr-mob">Телефон:</label><div class="col-md-4">
					...
					<button type="submit" name="save"  class="btn btn-primary">Сохранить</button></div>
					</div>
...
if(empty($address->error)){				
				...
				<form class="form-horizontal" action="addresses.php?id=<?php echo $id; ?>&action=delete" method="post" id="addr-del">
					...
					<div class="form-group"><div class="col-md-4 col-md-offset-3">
					<button class="btn btn-danger"  type="submit" name="delete">Удалить</button></div>
					</div>
...
?></select>
					...
					<label class="col-md-3 control-label text-right" for="addr-mob">Телейон:</label><div class="col-md-4">
...
<ul class="nav nav-pills">
		<li>
				 <a href="addresses.php?action=add" >Добавить Новые Данные</a></li>
				</ul>
				<hr>
...
<a class="btn btn-link" href="addresses.php?action=edit&id=<?php echo $all_address[$i]['id']; ?>">Изменить</a> <a class="btn btn-link" href="addresses.php?action=delete&id=<?php echo $all_address[$i]['id']; ?>">Delete</a><br>
</div>
<?php
}
}

...
/***/ function(module, exports, __webpack_require__) {
	...
	window.addEventListener('mousemove', function (event) {
	  if (!isTouchDevice) {
	    haze.gl.createUniform('2f', 'mouse', -curve(-1 + event.pageX / window.innerWidth * 3), -curve(-1 + event.pageY / window.innerHeight * 2));
	  }
	});
	
	window.addEventListener('devicemotion', function (event) {
	  if (isTouchDevice) {
	    haze.gl.createUniform('2f', 'mouse', curve(smoothX(-event.accelerationIncludingGravity.x / 10)) * 22, curve(smoothY(-event.accelerationIncludingGravity.y / 10)) * 13);
	  }
	});
